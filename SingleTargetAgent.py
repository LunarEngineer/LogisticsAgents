...
def prettyArr(a):
  """
  This silly function is just to pretty print numeric arrays
  in case you are doing some debugging.
  """
  for row in a:
    rowstr = ",".join(["{:.1f}".format(x) for x in row])
    print("\t{}".format(rowstr))
...
def getMission(trk,customer_needs,mission: int=None):
  ...
  if mission is not None:
    # print("Mission has been passed as {}".format(mission))
    cust = mission
  else:
  ...
  # if mission:
  #   print("Mission will be returned as {}".format(np.concatenate((np.atleast_1d(cust),pri))))
...
# Create an empty dictionary to store run information
outInfo = {}
...
while not done:
  ...
  # Update customer needs and 'undispatch' trucks if possible
  for trk_i in range(len(dispatch_status)):
    ...
    if customer_number < customer_needs.shape[0]:
      ...
      # How far from the customer is the truck?
      x_t, y_t = s[customer_number][0:2]
      x_0, y_0 = trk.location
      if sqrt((x_t-x_0)**2+(y_t-y_0)**2) < 1e-3:
        # If the truck is close to the target then give an 'illegal' target
        dispatch_status[trk_i][0] = customer_needs.shape[0] + 1
      else:
        # Otherwise update the needs for that customers line.
      ...
      # This is just here because the depot isn't a customer object
      #  and so it needs a different accounting for location.
      x_t, y_t = env.depot
      x_0, y_0 = trk.location
      if sqrt((x_t-x_0)**2+(y_t-y_0)**2) < 1e-3:
        dispatch_status[trk_i][0] = customer_needs.shape[0] + 1
  ...
  outInfo[env.current_step] = env.get_information()
  # This can be turned on to render
  # env.render()
env.close()
pd.DataFrame.from_dict(outInfo,orient='index').to_csv("out.csv")